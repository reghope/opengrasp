...
export async function startTui(opts: TuiOptions = {}) {
  ...
  function handleSlash(raw: string) {
    ...
    switch (cmd) {
      ...
      case "status":
        ...
        break;
      case "agent":
        if (rest[0]) state.agent = rest[0];
        log.log(`agent: ${state.agent}`);
        break;
      case "session":
        if (rest[0]) state.session = rest[0];
  ...
  function render() {
    ...
    footer.setContent(
      `Deliver: ${state.deliver ? "on" : "off"} | Model: ${state.model} | Thinking: off | Verbose: off | Reasoning: off`
    );
  }
