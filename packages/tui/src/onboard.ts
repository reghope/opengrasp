...
import type { AuthProvider, OnboardAuthEntry, OnboardResult } from "@opengrasp/shared";

const DEFAULT_WORKSPACE = "~/.opengrasp/workspace";
const DEFAULT_PROVIDERS: AuthProvider[] = ["anthropic", "openai-codex", "kimi"];

function normalizeProvider(input: string): AuthProvider | null {
  const value = input.trim().toLowerCase();
  if (value === "codex" || value === "openai-codex" || value === "openai") return "openai-codex";
  ...
  if (value === "venice") return "venice";
  if (value === "zai" || value === "z.ai") return "zai";
...
export async function runOnboardingTui(defaults: OnboardDefaults = {}): Promise<OnboardResult> {
  ...
  const ask = (question: string, opts?: { secret?: boolean; defaultValue?: string }): Promise<string> =>
    new Promise((resolve) => {
  ...
  if (extraProvider) {
    ...
    if (normalized && !providers.includes(normalized)) providers.push(normalized);
  }

  const authEntries: OnboardAuthEntry[] = [];
  for (const provider of providers) {
    const authType = (await ask(
      `Auth type for ${provider} (api/oauth/skip)?`,
      { defaultValue: "api" }
    )) as "api" | "oauth" | "skip";
    if (authType === "skip") continue;
    if (authType === "oauth") {
  ...
  const gatewayAuthMode = isAdvanced
    ? ((await ask("Gateway auth mode (token/password/none)?", { defaultValue: "token" })) as
        | "token"
        | "password"
        | "none")
