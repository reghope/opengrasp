...
import { ensureWorkspace, readStartupMemory, appendDailyMemory, appendLongTermMemory, estimateTokens, shouldFlushMemory } from "@opengrasp/memory";
...
async function readBootstrap(workspace) {
    const contents = [];
...
export async function runAgent(config, message, sessionKey) {
    ...
    if (config.agents.defaults.compaction.memoryFlush.enabled &&
        ...
        shouldFlushMemory(session.tokenEstimate, 128000, config.agents.defaults.compaction.reserveTokensFloor, config.agents.defaults.compaction.memoryFlush.softThresholdTokens)) {
